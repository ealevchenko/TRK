$(function () {
    ...
    var date_curent = new Date(),
        ...
        tab_type_reports = {
            ...
            activeTable: function (active, data_refresh) {
                if (active === 0) {
                    table_report_rw.viewTable(data_refresh);
                }
                if (active === 1) {
                    table_report_car.viewTable(data_refresh);
                }
        ...
        table_report_rw = {
            ...
            initObject: function () {
                this.obj = this.html_table.DataTable({
                    ...
                    "footerCallback": function (row, data, start, end, display) {
                        ...
                        table_report_rw.table_foot_html = "<tfoot>" +
                            "<tr>" +
                    ...
                    "columnDefs": [
                        { "visible": false, "targets": table_report_rw.groupColumn }
                    ],
                    "order": [[table_report_rw.groupColumn, 'asc']],
                    "drawCallback": function (settings) {
                        var api = this.api();
                        var rows = api.rows({ page: 'current' }).nodes();
                        var last = null;
                        var last_nak = null;
                        table_report_rw.table_html = "";
                        api.column([table_report_rw.groupColumn], { page: 'current' }).data().each(function (group, i) {
                            ...
                            if (last_nak !== num_nak) {
                                ...
                                table_report_rw.table_html += '<tr class="nakladnaya"><td colspan="10" class="left">Тип ГСМ - ' + type_fuel + ' Накладная №' + num_nak + '</td><td class="number">' + nak[0].sum + '</td><td></td><td></td><td></td><td></td></tr>';
                            ...
                            if (last !== group) {
                                ...
                                table_report_rw.table_html += '<tr class="group"><td colspan="15" class="left"> Цистерна №' + num_tanker + '</td></tr>';
                                ...
                                table_report_rw.table_html += '<tr class=""><td rowspan="' + tanker[0].count + '" class="number">' + nak_mass + '</td><td rowspan="' + tanker[0].count + '" class="number">' + manual_level + '</td><td rowspan="' + tanker[0].count + '" class="number">' + manual_volume + '</td><td rowspan="' + tanker[0].count + '" class="number">' + manual_dens + '</td><td rowspan="' + tanker[0].count + '" class="number">' + manual_mass + '</td><td>' + num + '</td><td>' + start_tank + '</td><td class="number">' + start_mass + '</td><td>' + stop_tank + '</td><td class="number">' + stop_mass + '</td><td class="number">' + change_mass + '</td><td rowspan="' + tanker[0].count + '" class="number">' + tanker[0].sum + '</td>' +
                                ...
                                table_report_rw.table_html += '<tr class=""><td>' + num + '</td><td>' + start_tank + '</td><td class="number">' + start_mass + '</td><td>' + stop_tank + '</td><td class="number">' + stop_mass + '</td><td class="number">' + change_mass + '</td></tr>';
                            }
            ...
            viewTable: function (data_refresh) {
                ...
                if (this.list === null | data_refresh === true) {
                    ...
                    getAsyncViewReportRFOfDateTime(
                        ...
                        function (result) {
                            table_report_rw.list = result;
                            table_report_rw.loadDataTable(result);
                            table_report_rw.obj.draw();
                        }
                    );
                } else {
                    table_report_rw.loadDataTable(this.list);
                    table_report_rw.obj.draw();
                };
            },
            // Загрузить данные
            ...
            exportTable: function () {
                ...
                var tab = "<table>" +
                    ...
                    "</tr>" +
                    "</thead>" +
                    "<tbody>" + table_report_rw.table_html + "</tbody>" + table_report_rw.table_foot_html + "</table>";
                fnExcelReport1("Ж.д. цистерны", tab, css, "Сменный отчёт приёма топлива в резервуары с " + toISOStringTZ(date_start).substring(0, 19) + " по "+toISOStringTZ(date_stop).substring(0, 19)); //
            }
        },

        table_report_car = {
            html_table: $('#table-report-car'),
            obj: null,
            select: null,
            select_id: null,
            list: [],
            ...
            initObject: function () {
                this.obj = this.html_table.DataTable({
                    ...
                    "footerCallback": function (row, data, start, end, display) {
                        ...
                        $('td#total-nakl-car').text(total_nakl);
                        //$('td#total-manual').text(total_manual);
                        $('td#total-change-car').text(total_change);
                        $('td#total-change-all-car').text(total_change);
                        //$('td#total-deviation-manual-sys').text(total_change !== 0 ? Number(((total_manual - total_change) / total_change) * 100.0).toFixed(3) : 0);
                        $('td#total-deviation-nakl-sys-car').text(total_change !== 0 ? Number(((total_nakl - total_change) / total_change) * 100.0).toFixed(3) : 0);
                        //$('td#total-deviation-manual-nakl').text(total_nakl !== 0 ? Number(((total_manual - total_nakl) / total_nakl) * 100.0).toFixed(3) : 0);
                    },
                    columns: [
                        { data: "group", title: "Группа", width: "50px", orderable: true, searchable: false },
                        { data: "fuel", title: "ГСМ", width: "50px", orderable: true, searchable: false, visible: false },
                        { data: "railway_num_nak", title: "Накладная", width: "50px", orderable: false, searchable: true, visible: false },
                        //{ data: "railway_num_tanker", title: "№ цистерны", width: "50px", orderable: false, searchable: true, visible: false },
                        //{ data: "railway_provider", title: "Поставщик", width: "150px", orderable: false, searchable: false, visible:false  },
                        { data: "railway_nak_mass", title: "Масса по накл (кг.)", width: "50px", orderable: false, searchable: false },
                        //{ data: "railway_manual_level", title: "Уровень р.з.", width: "50px", orderable: false, searchable: false },
                    ...
                    "columnDefs": [
                        { "visible": false, "targets": table_report_car.groupColumn }
                    ],
                    "order": [[table_report_car.groupColumn, 'asc']],
                    "drawCallback": function (settings) {
                        var api = this.api();
                        var rows = api.rows({ page: 'current' }).nodes();
                        var last = null;
                        var last_nak = null;
                        table_report_car.table_html = "";
                        api.column([table_report_car.groupColumn], { page: 'current' }).data().each(function (group, i) {

                            var type_fuel = api.column(1).data()[i];
                            var num_nak = api.column(2).data()[i];
                            var nak_mass = api.column(3).data()[i];
                            //var manual_level = api.column(5).data()[i];
                            //var manual_volume = api.column(6).data()[i];
                            //var manual_dens = api.column(7).data()[i];
                            //var manual_mass = api.column(8).data()[i];

                            var num = api.column(4).data()[i];
                            var start_tank = api.column(5).data()[i];
                            var start_mass = api.column(6).data()[i];
                            var stop_tank = api.column(7).data()[i];
                            var stop_mass = api.column(8).data()[i];
                            var change_mass = api.column(9).data()[i];

                            if (last_nak !== num_nak) {
                                var nak = getObjects(naks, 'num', num_nak);

                                $(rows).eq(i).before(
                                    '<tr class="nakladnaya"><td colspan="8" class="left">Тип ГСМ - ' + type_fuel + ' Накладная №' + num_nak + '</td><td>' + nak[0].sum + '</td></tr>'
                                );
                                table_report_car.table_html += '<tr class="nakladnaya"><td colspan="8" class="left">Тип ГСМ - ' + type_fuel + ' Накладная №' + num_nak + '</td><td class="number">' + nak[0].sum + '</td></tr>';

                                $(rows).eq(i).before(
                                    '<tr class=""><td rowspan="' + nak[0].count + '">' + nak_mass + '</td><td>' + num + '</td><td>' + start_tank + '</td><td>' + start_mass + '</td><td>' + stop_tank + '</td><td>' + stop_mass + '</td><td>' + change_mass + '</td><td rowspan="' + nak[0].count + '">' + nak[0].sum + '</td>' +
                                    '<td rowspan="' + nak[0].count + '">' + Number(((nak_mass - nak[0].sum) / nak[0].sum) * 100.0).toFixed(3) + '</td></tr>'
                                );
                                table_report_car.table_html += '<tr class=""><td rowspan="' + nak[0].count + '" class="number">' + nak_mass + '</td><td>' + num + '</td><td>' + start_tank + '</td><td class="number">' + start_mass + '</td><td>' + stop_tank + '</td><td class="number">' + stop_mass + '</td><td class="number">' + change_mass + '</td><td rowspan="' + nak[0].count + '" class="number">' + nak[0].sum + '</td>' +
                                    '<td rowspan="' + nak[0].count + '" class="number">' + Number(((nak_mass - nak[0].sum) / nak[0].sum) * 100.0).toFixed(3) + '</td></tr>';
                                ...
                                table_report_car.table_html += '<tr class=""><td>' + num + '</td><td>' + start_tank + '</td><td class="number">' + start_mass + '</td><td>' + stop_tank + '</td><td class="number">' + stop_mass + '</td><td class="number">' + change_mass + '</td></tr>';
                            }
                            ...
                            //    $(rows).eq(i).before(
                            //        //'<tr class="group"><td colspan="10"> Цистерна №' + num_tanker + '</td><td>' + tanker[0].sum + '</td><td></td><td></td><td></td><td></td></tr>'
                            //        '<tr class="group"><td colspan="10" class="left"> Цистерна №' + num_tanker + '</td></tr>'
                            //    );
                            //    table_report_car.table_html += '<tr class="group"><td colspan="10" class="left"> Цистерна №' + num_tanker + '</td></tr>';
                            //    var raz_treb = Number(tanker[0].sum - Number(nak_mass));
                            //    //var raz_manual = Number(tanker[0].sum - Number(manual_mass));
                            //    $(rows).eq(i).before(
                            //        //'<tr class=""><td rowspan="' + nak[0].count + '">' + nak_mass + '</td><td rowspan="' + nak[0].count + '">' + manual_level + '</td><td rowspan="' + nak[0].count + '">' + manual_volume + '</td><td rowspan="' + nak[0].count + '">' + manual_dens + '</td><td rowspan="' + nak[0].count +'">' + manual_mass + '</td><td colspan="5" class="razniza-txt">Расхождения накладная (ручные замеры) :</td><td class="razniza">' + raz_treb + '(' + raz_manual + ')' + '</td></tr>'
                            //        '<tr class=""><td rowspan="' + tanker[0].count + '">' + nak_mass + '</td><td>' + num + '</td><td>' + start_tank + '</td><td>' + start_mass + '</td><td>' + stop_tank + '</td><td>' + stop_mass + '</td><td>' + change_mass + '</td><td rowspan="' + tanker[0].count + '">' + tanker[0].sum + '</td>' +
                            //        '<td rowspan="' + tanker[0].count + '">' + Number(((nak_mass - tanker[0].sum) / tanker[0].sum) * 100.0).toFixed(3) + '</td></tr>'
                            //    );
                            //    table_report_car.table_html += '<tr class=""><td rowspan="' + tanker[0].count + '" class="number">' + nak_mass + '</td><td>' + num + '</td><td>' + start_tank + '</td><td class="number">' + start_mass + '</td><td>' + stop_tank + '</td><td class="number">' + stop_mass + '</td><td class="number">' + change_mass + '</td><td rowspan="' + tanker[0].count + '" class="number">' + tanker[0].sum + '</td>' +
                            //        '<td rowspan="' + tanker[0].count + '" class="number">' + Number(((nak_mass - tanker[0].sum) / tanker[0].sum) * 100.0).toFixed(3) + '</td></tr>';
                            //    last = group;
                            //} else {
                            //    $(rows).eq(i).before(
                            //        //'<tr class=""><td colspan="5"></td><td>' + num + '</td><td>' + start_tank + '</td><td>' + start_mass + '</td><td>' + stop_tank + '</td><td>' + stop_mass + '</td><td>' + change_mass + '</td></tr>'
            ...
            viewTable: function (data_refresh) {
                ...
                if (this.list === null | data_refresh === true) {
                    // Обновим данные
                    getAsyncViewReportRFOfDateTime(
                        0, date_start, date_stop,
                        function (result) {
                            table_report_car.list = result;
                            table_report_car.loadDataTable(result);
                            table_report_car.obj.draw();
                        }
                    );
                } else {
                    table_report_car.loadDataTable(this.list);
                    table_report_car.obj.draw();
                };
            },
            // Загрузить данные
            ...
            exportTable: function () {
                ...
                var tab = "<table>" +
                    ...
                    "</tr>" +
                    "</thead>" +
                    "<tbody>" + table_report_car.table_html + "</tbody>" + table_report_car.table_foot_html + "</table>";
                fnExcelReport1("Автоцистерна", tab, css, "Сменный отчёт приёма топлива в резервуары с " + toISOStringTZ(date_start).substring(0, 19) + " по "+toISOStringTZ(date_stop).substring(0, 19)); //
            }
        };
    ...
    table_report_rw.initObject();
    table_report_car.initObject();
    panel_select_report.viewReport();
